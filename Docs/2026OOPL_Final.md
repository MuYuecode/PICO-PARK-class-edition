...
遊戲開始前，須先依照 `README.md` 內的環境配置方式完成 CLion 與 CMake 設定，方可正常建置並執行遊戲。
...
### 問題與解決方法
物理模型在角色與可自行移動的物件互動時最容易出現問題，例如角色站上移動平台後，曾經發生穿模、抖動、瞬間被移動到錯誤位置，或平台上升時角色被不自然推出去的情況。這類問題不只是單純的座標錯誤，而是牽涉到更新順序、碰撞解析、支撐關係與移動平台本身的位移，並且 AI 亦較難直接認知到實際遊戲畫面中發生的異常。

解決方式是將物理流程拆分為「先計算預期位移、再解析碰撞、最後套用位移」，並加入 `SupportResolver` 偵測角色是否站在平台或其他物件上，使平台位移能正確傳遞給角色；同時使用 `CollisionSolver` 分別處理水平與垂直方向的碰撞，以減少穿模和抖動。

第二個問題是場景切換。遊戲不僅需要一般的畫面切換，亦需要暫停選單覆蓋在關卡之上，並且在繼續遊戲時保留原本關卡狀態。因此 `SceneManager` 使用 stack 管理場景，讓 `LevelExitScene` 可以暫停底層關卡、重啟關卡或清空堆疊回到其他畫面。
...
第三個問題是經過測試後，部分電腦在使用學校有線網路與 CLion 2026.1.3 版本執行 Load CMake Project 時會出現 TLS 錯誤，導致 PTSD 引擎無法被正常下載。

最快的解決方法為在 File > Settings > Build, Execution, Deployment > CMake > Environment 加入 `CMAKE_TLS_VERIFY=0`，並執行 Tools > CMake > Reset Cache and Reload Project。

最後一個問題是部分薄膜鍵盤（如 Dell KB525C）可能無法正確讀取多個鍵位同時輸入，導致遊戲人物的實際操作與玩家預想中的操作不相符。

解決方法為先在 [鍵盤測試網站](https://zhouer.org/KeyboardTest/) 內測試（例如按著 `D` + `RIGHT` 或 `A` + `LEFT` 的情況下，是否能同時讀取到 `W` + `UP`），或是外接機械鍵盤。
