...
#include "gameplay/actors/PushableBox.hpp"
...
void PushableBox::PhysicsUpdate() {
    ...
    int netRight = 0;
    int netLeft  = 0;
    if (m_PushQuery != nullptr) {
        netRight = m_PushQuery->CountCharactersPushing(this, +1);
        netLeft  = m_PushQuery->CountCharactersPushing(this, -1);
    }
...
void PushableBox::NotifyAdjacentPushers(int activeDir) const {
    if (m_PushQuery == nullptr) return;

    const auto chars = m_PushQuery->GetBodiesOfType(BodyType::CHARACTER);
